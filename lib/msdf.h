...
typedef struct msdf_AllocCtx {
  ...
  void (*free_msdf_AllocCtx)(void* ptr, void* ctx);
  void* ctx;
} msdf_AllocCtx;
...
int msdf_genGlyph(msdf_Result* result, stbtt_fontinfo *font, int stbttGlyphIndex, uint32_t borderWidth, float scale, float range, msdf_AllocCtx* alloc) {
    ...
    if (alloc) {
        ...
        allocCtx.free_msdf_AllocCtx = msdf__free;
        allocCtx.ctx = NULL;
    }
    ...
    for (int i = 0; i < contour_count; ++i) {
        ...
        if (cornerIndex == 0) {
            ...
            if (contour_data[i].edge_count >= 3) {
                ...
                if (allocCtx.free_msdf_AllocCtx) {
                    allocCtx.free_msdf_AllocCtx(contour_data[i].edges, allocCtx.ctx);
                }
    ...
    if (allocCtx.free_msdf_AllocCtx) {
        allocCtx.free_msdf_AllocCtx(corners, allocCtx.ctx);
    }
    ...
    if (allocCtx.free_msdf_AllocCtx) {
        ...
        allocCtx.free_msdf_AllocCtx(contour_data, allocCtx.ctx);
        allocCtx.free_msdf_AllocCtx(contour_sd, allocCtx.ctx);
        allocCtx.free_msdf_AllocCtx(contours, allocCtx.ctx);
        allocCtx.free_msdf_AllocCtx(windings, allocCtx.ctx);
        allocCtx.free_msdf_AllocCtx(verts, allocCtx.ctx);
    }
    ...
    if (allocCtx.free_msdf_AllocCtx) {
        allocCtx.free_msdf_AllocCtx(clashes, allocCtx.ctx);
    }
