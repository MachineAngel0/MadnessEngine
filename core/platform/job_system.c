...
void store_result(fpn_job_on_complete callback, u32 param_size, void* params) {
    ...
    if (!mutex_lock(&state_ptr->result_mutex)) {
    ...
    if (!mutex_unlock(&state_ptr->result_mutex)) {
        M_ERROR("Failed to release mutex lock for result storage, storage may be corrupted.");
    }
}
...
u32 job_thread_run(void* params) {
    ...
    TRACE("Starting job thread #%i (id=%#x, type=%#x).", thread->index, thread_id, thread->type_mask);

    // A mutex to lock info for this thread.
    if (!mutex_create(&thread->info_mutex)) {
        M_ERROR("Failed to create job thread mutex! Aborting thread.");
        return 0;
    }
    ...
    while (true) {
        ...
        if (!mutex_lock(&thread->info_mutex)) {
            M_ERROR("Failed to obtain lock on job thread mutex!");
        }
        job_info info = thread->info;
        if (!mutex_unlock(&thread->info_mutex)) {
            M_ERROR("Failed to release lock on job thread mutex!");
        }
...
bool job_system_initialize(u64* job_system_memory_requirement, void* state, u8 job_thread_count, u32 type_masks[]) {
    *job_system_memory_requirement = sizeof(job_system_state);
    ...
    DEBUG("Main thread id is: %#x", get_thread_id());

    DEBUG("Spawning %i job threads.", state_ptr->thread_count);

    for (u8 i = 0; i < state_ptr->thread_count; ++i) {
        state_ptr->job_threads[i].index = i;
        state_ptr->job_threads[i].type_mask = type_masks[i];
        if (!thread_create(job_thread_run, &state_ptr->job_threads[i].index, false, &state_ptr->job_threads[i].thread)) {
            FATAL("OS Error in creating job thread. Application cannot continue.");
            return false;
        }
    ...
    if (!mutex_create(&state_ptr->result_mutex)) {
        M_ERROR("Failed to create result mutex!.");
        return false;
    }
    if (!kmutex_create(&state_ptr->low_pri_queue_mutex)) {
        M_ERROR("Failed to create low priority queue mutex!.");
        return false;
    }
    if (!kmutex_create(&state_ptr->normal_pri_queue_mutex)) {
        M_ERROR("Failed to create normal priority queue mutex!.");
        return false;
    }
    if (!kmutex_create(&state_ptr->high_pri_queue_mutex)) {
        M_ERROR("Failed to create high priority queue mutex!.");
        return false;
    }
...
void job_system_shutdown(void* state) {
    if (state_ptr) {
        ...
        for (u8 i = 0; i < thread_count; ++i) {
            thread_destroy(&state_ptr->job_threads[i].thread);
        }
...
void process_queue(ring_queue* queue, Madness_Mutex* queue_mutex) {
    u64 thread_count = state_ptr->thread_count;
...
job_info job_create(fpn_job_start entry_point, fpn_job_on_complete on_success, fpn_job_on_complete on_fail, void* param_data, u32 param_data_size, u32 result_data_size) {
...
job_info job_create_type(fpn_job_start entry_point, fpn_job_on_complete on_success, fpn_job_on_complete on_fail, void* param_data, u32 param_data_size, u32 result_data_size, job_type type) {
...
job_info job_create_priority(fpn_job_start entry_point, fpn_job_on_complete on_success, fpn_job_on_complete on_fail, void* param_data, u32 param_data_size, u32 result_data_size, job_type type, job_priority priority) {
    job_info job;
