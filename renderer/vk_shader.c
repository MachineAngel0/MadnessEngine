    ...
    vulkan_pipeline_cache* pipeline_cache)
{
    ...
    VkPushConstantRange push_constant = {0};
    push_constant.offset = 0;
    push_constant.size = sizeof(PC_Skinned_Mesh);
    push_constant.stageFlags = VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT;
    ...
    VkResult pipeline_result = vkCreatePipelineLayout(renderer->context.device.logical_device, &pipeline_layout_info,
                                                      NULL,
                                                      &skinned_mesh_pipeline->pipeline_layout);
    VK_CHECK(pipeline_result);
    //graphics pipeline
    file_read_data vert_data = {0};
    file_read_data frag_data = {0};

    filesystem_open_and_return_bytes("../renderer/shaders/skinned_mesh.vert.spv", &vert_data);
    filesystem_open_and_return_bytes("../renderer/shaders/skinned_mesh.frag.spv", &frag_data);
    if (vert_data.size == 0)
    {
        FATAL("Vertex shader file not loaded! Size: %llu", vert_data.size);
    }
 ...
 */
    ...
    //
    //     spirv_reflect_input_variable_info* attribute_info =
    //     spriv_reflect_get_input_variable(NULL, "../renderer/shaders/skinned_mesh.vert.spv");
    // u32 offset_total = 0;
    //     VkVertexInputAttributeDescription* attribute_descriptions = malloc(
    //         sizeof(VkVertexInputAttributeDescription) * attribute_info->input_count);
    //     for (u32 attribute_index = 0; attribute_index < attribute_info->input_count; attribute_index++)
    ...
    */
    ...
    graphics_pipeline_info.pColorBlendState = &color_blending;
    graphics_pipeline_info.pDynamicState = &dynamicState;
    graphics_pipeline_info.layout = skinned_mesh_pipeline->pipeline_layout;
    graphics_pipeline_info.renderPass = 0; // this has to be null if we are doing dynamic rendering
    // graphics_pipeline_info.subpass = 0;
    // graphics_pipeline_info.basePipelineHandle = VK_NULL_HANDLE;
    // graphics_pipeline_info.basePipelineIndex = -1;
...
bool sprite_shader_create(Renderer* renderer, vulkan_shader_pipeline* sprite_pipeline, vulkan_pipeline_cache* pipeline_cache)
{
  // Pipeline layout creation
    ...
    VkResult pipeline_result = vkCreatePipelineLayout(renderer->context.device.logical_device, &pipeline_layout_info,
                                                      NULL,
                                                      &sprite_pipeline->pipeline_layout);
    VK_CHECK(pipeline_result);
    //graphics pipeline
    file_read_data vert_data = {0};
    file_read_data frag_data = {0};

    filesystem_open_and_return_bytes("../renderer/shaders/sprite.vert.spv", &vert_data);
    filesystem_open_and_return_bytes("../renderer/shaders/sprite.frag.spv", &frag_data);
    if (vert_data.size == 0)
    {
        FATAL("Vertex shader file not loaded! Size: %llu", vert_data.size);
    }
    ...
    spirv_reflect_input_variable_info* attribute_info =
        spriv_reflect_get_input_variable(NULL, "../renderer/shaders/sprite.vert.spv");
    u32 offset_total = 0;
    VkVertexInputAttributeDescription* attribute_descriptions = malloc(
        sizeof(VkVertexInputAttributeDescription) * attribute_info->input_count);
    for (u32 attribute_index = 0; attribute_index < attribute_info->input_count; attribute_index++)
