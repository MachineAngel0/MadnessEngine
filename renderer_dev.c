...
{
    renderer_app->renderer_application.renderer_dll_handle = platform_load_dynamic_library("./MADNESSRENDERER");
    DLL_HANDLE render_lib_handle = renderer_app->renderer_application.renderer_dll_handle;
...
bool renderer_dev_run(Renderer_Dev_Application* render_dev_app)
{
    ...
    Madness_UI* madness_ui = madness_ui_init(&render_dev_app->application_base.memory_system, renderer);
    ...
    while (render_dev_app->application_base.is_running)
    {
        ...
        if (platform_has_filed_changed(renderer_thing_handle))
        {
            ...
            platform_reload_dynamic_library(render_dev_app->renderer_application.renderer_dll_handle);
        }
