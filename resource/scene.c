...
Scene* scene_init(Memory_System* memory_system)
{
    ...
    scene->mesh_transforms = memory_system_alloc(memory_system, sizeof(Transform) * MAX_TRANSFORM_COUNT,
                                            MEMORY_SUBSYSTEM_RESOURCE);
    scene->mesh_world_transforms = memory_system_alloc(memory_system, sizeof(mat4) * MAX_TRANSFORM_COUNT,
                                            MEMORY_SUBSYSTEM_RESOURCE);

    scene->mesh_transform_count = 0;
...
{
    MASSERT(scene);
    memory_system_memory_free(memory_system, scene->mesh_transforms);
...
Transform_Handle scene_get_new_mesh_transform(Scene* scene)
{
    ...
    Transform_Handle out_handle = {
        .handle = scene->mesh_transform_count,
        .gen = 0,
    };
    scene->mesh_transforms[scene->mesh_transform_count];
    scene->mesh_transform_count++;
...
void scene_update(Scene* scene, Resource_System* resource_system)
{
    ...
    for (int i = 0; i < scene->mesh_transform_count; ++i)
    {
        scene->mesh_world_transforms[i] = transform_get_world(scene->mesh_transforms);
    }

    resource_system->render_packet->mesh_data_packet.world_space_matrix_array = scene->mesh_world_transforms;
    resource_system->render_packet->mesh_data_packet.world_space_matrix_count = scene->mesh_transform_count;
